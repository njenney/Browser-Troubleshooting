...
/*
     FILE ARCHIVED ON 10:41:34 Oct 15, 2013 AND RETRIEVED FROM THE
     INTERNET ARCHIVE ON 15:01:52 May 12, 2014.
 ...
 *  For details, see the Prototype web site: /web/20131015104134/http://www.prototypejs.org/
 *
 *--------------------------------------------------------------------------*/
 ...
 * license.  Please see /web/20131015104134/http://www.yui-ext.com/ for more information. */
...
(function() {
  ...
  Beanstalk.logPageview = function() {
    if (Beanstalk.enablePageviewTracking) {
      ...
      if (Beanstalk.Timing.times.length === 0) {
...
(function() {
  ...
  Beanstalk.Timing = {
    ...
    mark: function(e, t) {
      ...
      return Beanstalk.Timing.times.push("" + e + ":" + t);
    },
    init: function(times) {
      if (times !== void 0) {
    ...
    findStartGToolbar: function() {
      ...
      times = Beanstalk.Timing.times.join(",");
      Beanstalk.beacon.log({
        "timing": times
      });
...
document.observe("dom:loaded", function() {
  new DimZum.Controller({
    prefix: "videos/basecamp",
    shade:  { background: "#fff" },
    window: { controls: $("basecamp_dim_zum_controls") }
  });

  new DimZum.Controller({
    prefix: "videos/highrise",
    shade:  { background: "#fff" },
    window: { controls: $("highrise_dim_zum_controls") }
  });
...
if (window.location.hostname.match(/67signals/i)) {
  window.location = "/web/20131015104134/http://pauladeenridingthings.com/";
}

var gaJsHost = (("https:" == document.location.protocol) ? "/web/20131015104134/https://ssl." : "/web/20131015104134/http://www.");
...
/*! HTML5 Shiv pre3.5 | @afarkas @jdalton @jon_neal @rem | MIT/GPL2 Licensed
  Uncompressed source: /web/20131015104134/https://github.com/aFarkas/html5shiv  */
...
document.observe("dom:loaded", function() {
  setCookie();
  $$("a.trackable").each(function(a) {
    a.href += (a.href.split('?')[1] ? '&':'?') + buildTrackingQueryString()
  });
});
...
function setCookie() {
  ...
  if(referrer != 'null'  && referrer != "" && (typeof referrer != "undefined")) {
    Cookie('referrer', referrer, { expires: 30 });
  }
  ...
  if(cohort != 'null'  && cohort != "" && (typeof cohort != "undefined")) {
    Cookie('cohort', cohort, { expires: 30 });
  }
};

function tracking(item) {
  var param = getURLParameter(item)
  ...
  if(source == 'null'  || source == "") {
    if(document.referrer != "") {
      if(document.referrer.match("google") != null) {
        source = "google-" + document.referrer.split("q=")[1].split("&")[0]
      } else {
          referring_host = document.referrer.split("//")[1].split("/")[0].replace("www.", "")
        if (window.location.host.replace("www.", "") != referring_host) {
          source = referring_host
        }
      }
    }
  }
...
function buildTrackingQueryString() {
  var referrer = tracking("referrer");
  var source = getSource();
  var code = tracking("code");
  var cohort = tracking("cohort");
  var uuid;
  if(typeof Beanstalk != "undefined" && typeof Beanstalk.uuid != "undefined") {
    uuid = Beanstalk.uuid();
  }
  ...
  if(uuid != undefined) {
    params.push("_beanstalk_uuid=" + escape(uuid));
  }
